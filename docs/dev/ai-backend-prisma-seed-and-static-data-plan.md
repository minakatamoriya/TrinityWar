...
1. Prisma/schema 规划：[dev/ai-backend-prisma-schema-plan.md](dev/ai-backend-prisma-schema-plan.md)
2. Migration 切分计划：[dev/ai-backend-migration-splitting-plan.md](dev/ai-backend-migration-splitting-plan.md)
3. 读接口切换计划：[dev/ai-backend-read-api-cutover-plan.md](dev/ai-backend-read-api-cutover-plan.md)
4. 前端 mock 退场计划：[dev/frontend-mock-retirement-plan.md](dev/frontend-mock-retirement-plan.md)
5. 农场设计主文档：[docs/design/launch-farm-and-gold-plan-first-pass.md](docs/design/launch-farm-and-gold-plan-first-pass.md)
...
建议优先从 [docs/design/launch-farm-and-gold-plan-first-pass.md](docs/design/launch-farm-and-gold-plan-first-pass.md) 的首发种子表整理，而不是继续直接抄 [apps/game-client/src/api.ts](apps/game-client/src/api.ts) 里的临时常量。
...
2. 再让真实接口把种子相关字段吐给前端。
3. 最后按 [dev/frontend-mock-retirement-plan.md](dev/frontend-mock-retirement-plan.md) 逐步移除前端临时种子表。
...
你现在只负责生成 SeedDefinition 的主数据 seed 文件。请以 [docs/design/launch-farm-and-gold-plan-first-pass.md](docs/design/launch-farm-and-gold-plan-first-pass.md) 为主来源，以 [dev/ai-backend-prisma-seed-and-static-data-plan.md](dev/ai-backend-prisma-seed-and-static-data-plan.md) 为执行边界。不要顺手生成 PlayerSeedInventory 初始化逻辑。
