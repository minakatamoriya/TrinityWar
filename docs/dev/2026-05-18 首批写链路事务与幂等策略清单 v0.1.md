...
3. Service 分层：[dev/2026-05-18 后端 Service 分层设计 v0.1.md](dev/2026-05-18 后端 Service 分层设计 v0.1.md)
4. 模块骨架：[dev/2026-05-18 首批 NestJS 模块拆分与目录骨架规划 v0.1.md](dev/2026-05-18 首批 NestJS 模块拆分与目录骨架规划 v0.1.md)
...
你现在只负责生成 [动作名] 命令链路。必须严格遵循 [dev/2026-05-18 首批写链路事务与幂等策略清单 v0.1.md](dev/2026-05-18 首批写链路事务与幂等策略清单 v0.1.md) 中该动作的事务边界、幂等策略、版本校验点和日志落点。不要顺手实现其他命令。

### 2. 检查现有命令链路

你现在只负责检查 [动作名] 命令链路是否符合 [dev/2026-05-18 首批写链路事务与幂等策略清单 v0.1.md](dev/2026-05-18 首批写链路事务与幂等策略清单 v0.1.md)。请逐项核对：事务内操作、事务外操作、版本校验、幂等处理、审计日志、错误码和重试行为。
