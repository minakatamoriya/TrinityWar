...
SELECT
  'first_stipend_plant_' || p."id" || '_' || sd."seed_id",
  p."id",
  sd."id",
  0,
  1,
...
WHERE sd."seed_id" IN ('qinglingmai', 'xunyamai')
  AND EXISTS (
    SELECT 1
    FROM "player_faction_stipend_state" pfss
    WHERE pfss."player_id" = p."id"
      AND pfss."claimed_at" IS NOT NULL
  )
...
SELECT
  'first_stipend_research_' || p."id" || '_' || sd."seed_id",
  p."id",
  sd."id",
  CURRENT_TIMESTAMP,
  1,
