import { getSpiritBattleInnateRules, SPIRIT_BATTLE_TRAIT_BY_CODE, } from './spiritBattleRules.js';
export const DEFAULT_SPIRIT_COLLISION_BATTLE_CONFIG = {
    maxRounds: 10,
    baseStealRatio: 0.2,
...
export function buildSpiritCollisionBattleReplay(input) {
    ...
    const events = [];
    const steps = [{ type: 'enter', durationMs: 520 }];
    let result = null;
    let resultReason = '';
    ...
    for (let round = 1; round <= config.maxRounds; round += 1) {
        ...
        appendRoundSteps(steps, round, attackerBefore, defenderBefore, attacker.hp, defender.hp, attacker.snapshot.maxHp, defender.snapshot.maxHp, attackerDamage, defenderDamage, heals, config);
    ...
    if (!result) {
        ...
        for (let bloodRound = 1; bloodRound <= config.maxBloodModeRounds && !result; bloodRound += 1) {
            ...
            const bloodResult = resolveBloodModeDeaths(attacker.hp, defender.hp, bloodRound, attackerTotalCombatDamage, defenderTotalCombatDamage, attackerBloodEntryHpRatio, defenderBloodEntryHpRatio, '燃血结算后');
            if (bloodResult) {
            ...
            appendRoundSteps(steps, round, attackerBefore, defenderBefore, attacker.hp, defender.hp, attacker.snapshot.maxHp, defender.snapshot.maxHp, attackerDamage, defenderDamage, heals, config);
            appendRoundEvents(events, round, attackerDamage, defenderDamage, heals, attacker.hp, defender.hp, attackerTotalCombatDamage, defenderTotalCombatDamage);
            const clashResult = resolveBloodModeDeaths(attacker.hp, defender.hp, bloodRound, attackerTotalCombatDamage, defenderTotalCombatDamage, attackerBloodEntryHpRatio, defenderBloodEntryHpRatio, '普通互撞后');
            if (clashResult) {
    ...
    if (!result) {
        const tieBreak = resolveBloodModeTieBreak(attackerTotalCombatDamage, defenderTotalCombatDamage, attackerBloodEntryHpRatio, defenderBloodEntryHpRatio);
    ...
    const stealRatio = clampRatio(config.baseStealRatio
        + defenderLostHpRatio * config.defenderLostHpStealFactor
        + (result === 'WIN' ? config.attackerWinBonus : 0), config.minStealRatio, config.maxStealRatio);
...
function buildRoundContext(round, bloodRound, attacker, defender) {
    return {
        round,
        bloodRound,
...
function resolveRoundAttackPercent(attacker, defender, context, labels) {
...
function resolveRoundCritPercent(attacker, defender, context, labels) {
...
function resolveFinalDamagePercent(attacker, defender, context, labels) {
...
function resolveDamageTakenPercent(defender, attacker, context, labels) {
    return resolveInnatePercent(defender, attacker, context, 'damageTakenPercent', labels);
}
function resolveRoundLifestealPercent(attacker, defender, context, labels) {
...
function resolveRoundHeals(attacker, defender, round, attackerLifesteal, defenderLifesteal) {
...
function resolveFixedRoundHeal(unit, round) {
...
function resolveUnitBloodLossRatio(unit, opponent, baseRatio) {
...
function resolveTraitEffectValue(traits, stat, unit, opponent, context, labels, options = {}) {
    return Object.entries(traits).reduce((total, [code, traitValue]) => {
        const definition = SPIRIT_BATTLE_TRAIT_BY_CODE[code];
...
function hasRuntimeCondition(rule) {
    return Boolean(rule.activeRounds
...
function isRuleConditionActive(rule, unit, opponent, context) {
    if (rule.activeRounds && context && !rule.activeRounds.includes(context.round))
        return false;
    if (typeof rule.minRound === 'number' && context && context.round < rule.minRound)
        return false;
    if (typeof rule.maxRound === 'number' && context && context.round > rule.maxRound)
        return false;
    if (rule.bloodOnly && context?.bloodRound === null)
        return false;
    if (!unit || !opponent)
        return !hasRuntimeCondition(rule);
    ...
    if (typeof rule.selfHpBelowRatio === 'number' && selfHpRatio >= rule.selfHpBelowRatio)
        return false;
    if (typeof rule.selfHpAboveRatio === 'number' && selfHpRatio <= rule.selfHpAboveRatio)
        return false;
    if (typeof rule.targetHpBelowRatio === 'number' && targetHpRatio >= rule.targetHpBelowRatio)
        return false;
    if (rule.targetMaxHpHigher && opponent.snapshot.maxHp <= unit.snapshot.maxHp)
        return false;
    if (rule.selfHpLowerThanTarget && unit.hp >= opponent.hp)
        return false;
    return true;
}
function resolveElementDamageState(attacker, defender) {
    if (attacker && defender && controlsElement(attacker, defender))
        return 'strong';
    if (attacker && defender && controlsElement(defender, attacker))
        return 'weak';
    return 'none';
}
function controlsElement(left, right) {
    return ((left === 'metal' && right === 'wood')
...
function resolveRoundDamage(attacker, defender, context, config, random) {
...
function appendRoundSteps(steps, round, attackerBefore, defenderBefore, attackerAfter, defenderAfter, attackerMaxHp, defenderMaxHp, attackerDamage, defenderDamage, heals, config) {
...
function appendRoundEvents(events, round, attackerDamage, defenderDamage, heals, attackerHp, defenderHp, attackerTotalCombatDamage, defenderTotalCombatDamage) {
...
function resolveBloodLoss(maxHp, ratio) {
    if (ratio <= 0)
        return 0;
    return Math.max(Math.round(Math.max(maxHp, 1) * ratio), 1);
}
function resolveBloodLossRatio(bloodRound, config) {
    return Math.max(config.bloodModeInitialHpLossRatio + Math.max(Math.floor(bloodRound) - 1, 0) * config.bloodModeHpLossRatioIncrement, 0);
}
function formatBloodRatio(ratio) {
    const percent = ratio * 100;
    return `${Number.isInteger(percent) ? percent.toFixed(0) : percent.toFixed(1)}%`;
}
function capEffectiveDamage(targetHpBefore, rawDamage) {
    return Math.min(Math.max(Math.round(targetHpBefore), 0), Math.max(Math.round(rawDamage), 0));
}
function resolveBloodModeDeaths(attackerHp, defenderHp, bloodRound, attackerTotalCombatDamage, defenderTotalCombatDamage, attackerBloodEntryHpRatio, defenderBloodEntryHpRatio, timing) {
...
function resolveBloodModeTieBreak(attackerTotalCombatDamage, defenderTotalCombatDamage, attackerBloodEntryHpRatio, defenderBloodEntryHpRatio) {
