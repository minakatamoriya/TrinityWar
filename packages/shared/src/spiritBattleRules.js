export const SPIRIT_BATTLE_TRAIT_DEFINITIONS = [
...
export const LEGACY_SPIRIT_BATTLE_TRAIT_DEFINITIONS = [
    { code: 'counter', label: '反击', value: 10, description: '旧版词条，首发新战斗暂不生效' },
    { code: 'tenacity', label: '韧性', value: 10, description: '旧版词条，首发新战斗暂不生效' },
];
export const SPIRIT_BATTLE_TRAIT_BY_CODE = Object.fromEntries(SPIRIT_BATTLE_TRAIT_DEFINITIONS.map((definition) => [definition.code, definition]));
export const SPIRIT_BATTLE_TRAIT_LABELS = Object.fromEntries([...SPIRIT_BATTLE_TRAIT_DEFINITIONS, ...LEGACY_SPIRIT_BATTLE_TRAIT_DEFINITIONS].map((definition) => [definition.code, definition.label]));
export const SPIRIT_BATTLE_INNATE_RULES = {
    canglang: [
...
export function getSpiritBattleInnateRules(spiritId) {
    return spiritId ? SPIRIT_BATTLE_INNATE_RULES[spiritId] ?? [] : [];
}
export function getSpiritBattleTraitLabel(code) {
